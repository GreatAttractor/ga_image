...
const _TAG_TYPE_BYTE:     u16 = 1;
const _TAG_TYPE_ASCII:    u16 = 2;
const TAG_TYPE_WORD:      u16 = 3; // 16 bits
const TAG_TYPE_DWORD:     u16 = 4; // 32 bits
const _TAG_TYPE_RATIONAL: u16 = 5;
...
pub fn load_tiff(file_name: &str) -> Result<Image, TiffError> {
    ...
    let bytes_per_line = img_width.unwrap() as usize * pix_fmt.bytes_per_pixel();
    ...
    let mut img = Image::new_from_pixels(img_width.unwrap(), img_height.unwrap(), None, pix_fmt, None, pixels);
...
pub fn save_tiff(img: &ImageView, file_name: &str) -> Result<(), TiffError> {
    ...
    field = TiffField { tag: TAG_ROWS_PER_STRIP,
                        ftype: TAG_TYPE_WORD,
                        count: 1,
                        value: img.height() as u32 }; // There is only one strip for the whole image
    if is_be { field.value <<= 16; }
    utils::write_struct(&field, &mut file)?;
