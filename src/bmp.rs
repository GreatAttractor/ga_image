...
///
fn bmp_metadata_priv(file: &mut File) -> Result<(u32, u32, usize, PixelFormat, Option<Palette>), BmpError> {
...
pub fn load_bmp<P: AsRef<Path>>(file_path: P) -> Result<Image, BmpError> {
    ...
    let dest_bytes_per_line = img_width as usize * pix_fmt.bytes_per_pixel();
    ...
    Ok(Image::new_from_pixels(img_width, img_height, None, pix_fmt, palette, pixels))
}
...
pub fn save_bmp<P: AsRef<Path>>(img: &ImageView, file_path: P) -> Result<(), BmpError> {
    ...
    let bytes_per_pix = pix_fmt.bytes_per_pixel();
    ...
    if [PixelFormat::Pal8, PixelFormat::Mono8].contains(&pix_fmt) {
        ...
        if pix_fmt == PixelFormat::Pal8 {
            let img_pal: &Palette = img.palette().iter().next().unwrap();
    ...
    for i in 0..height {
        let src_line = img.line_raw(height - i - 1);
